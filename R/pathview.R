...
invisible(lapply(list.of.packages, library, character.only = TRUE))
    ...
    for(i in 1:npath){
      ...
      if(kegg.native) tfiles=c(tfiles.xml[i],tfiles.png[i]) else {tfiles=tfiles.xml[i]}
      ...
      if((!is.null(gene.data) |map.null) & sum(node.data$type==gene.node.type)>1){
        ...
        if (rna.after.prot == T){
          #rna.color = c("purple","#C7594B","orange")
          cols.ts.gene.rna=node.color(plot.data.gene[,c(1:8,10)], limit$gene, bins$gene, both.dirs=both.dirs$gene, trans.fun=trans.fun$gene, 
                                  discrete=discrete$gene, low=rna.color[1], mid=rna.color[2], high=rna.color[3],  na.col=na.col)
          cols.ts.gene[,2]<- cols.ts.gene.rna
        }
